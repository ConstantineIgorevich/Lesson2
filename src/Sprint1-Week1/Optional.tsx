...
export function FirstWeekOptional() {
  
  const data1: PropsTasksType = {
    title: "What to do",
    tasks: [
...
function Tasks({ title, tasks, students }: PropsTasksType) {
  ...
  const studentsElements = students.map((el, i) => <li key={i}>{el}</li>);

  return (
    <div>
      <h3>{title}</h3>
      <ul>{tasksElements}</ul>
      <ul>{studentsElements}</ul>
    </div>
  );
}
